...
|---|---|---|
| `VERIFIED` | Supported by ≥1 authoritative independent source | Tier-A source, extraction confidence ≥ threshold, no unresolved conflict, freshness within type's TTL; user-facing rendering always carries the evidence as-of date |
...
### 13.6 External challenge & corpus governance
Internal evals are necessary and insufficient — we grade our own homework with our own answer key. Standing requirements: (1) **pre-launch external red-team**: an independent technical reviewer runs ≥50 real deals against **raw outputs** (never curated examples) and publishes findings to the data room unedited (mvp acceptance gate 7, `../roadmap/mvp.md` §4); repeated annually and offered to any lead investor's technical advisor (`../investor/due-diligence.md` §4). (2) **Corpus datasheets under external review**: provenance, licensing, label-guide version, inter-labeler agreement, and coverage gaps are documented per corpus version and open to diligence reproduction — accuracy claims inherit the credibility of their answer key, so the answer key gets audited too. (3) **Raw-output policy**: no eval result is ever reported on filtered or regenerated outputs; the number you see is the number the pipeline produced.
