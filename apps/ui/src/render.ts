...
function evidenceChip(evidence: EvidenceItem): string {
...
export function renderDealPage(run: VerificationRun): string {
  const unknowns = run.claims.filter(
    (claim) =>
  ...
  const memoSections = run.memo.sections
    .map(
      (section) =>
        `<section><h3>${escapeHtml(section.heading)}</h3><p>${escapeHtml(section.text)}</p></section>`,
    )
    .join("\n");
  ...
  const drillDown = run.claims
    .map(
      (claim) =>
        `<article id="claim-${claim.id}"><h4>${escapeHtml(claim.type)} — ${claim.status}</h4><p>${escapeHtml(claim.statusReason)}</p><p>Evidence refs: ${escapeHtml(claim.evidenceRefs.join(", ") || "none")}</p></article>`,
    )
    .join("\n");
