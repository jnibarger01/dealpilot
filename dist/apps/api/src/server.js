    ...
    return createServer(async (request, response) => {
        try {
            ...
            if (method === "GET" && url.pathname === "/") {
                sendHtml(response, 200, "<!doctype html><title>DealPilot MVP</title><h1>DealPilot MVP</h1><p>Evidence first. Unknowns are explicit.</p>");
                return;
            }
            if (method === "POST" && url.pathname === "/v1/deals") {
                const body = asRecord(await readJson(request));
                const artifactText = String(body.artifact_text ?? body.artifactText ?? "");
                const sourceUrl = typeof body.source_url === "string" ? body.source_url : undefined;
                const deal = await app.createDeal(sourceUrl
            ...
            if (method === "GET" &&
                ...
                if (!deal)
                    return problem(response, 404, "Deal not found.");
            ...
            if (method === "POST" &&
                ...
                const run = await app.startVerificationRun(parts[2] ?? "", {
                    ...
                    budgetCredits: typeof body.budget_credits === "number" ? body.budget_credits : 12,
                });
            ...
            if (method === "GET" &&
                ...
                if (!run)
                    return problem(response, 404, "Run not found.");
            ...
            if (method === "GET" &&
                ...
                parts[3] === "claims") {
                send(response, 200, {
                    data: app.getClaimsForRun(parts[2] ?? "", url.searchParams.get("status") ?? undefined),
                });
                return;
            }
            if (method === "GET" &&
                ...
                if (!claim)
                    return problem(response, 404, "Claim not found.");
            ...
            if (method === "POST" &&
                ...
                const evidence = await app.addManualEvidence(parts[2] ?? "", {
                    ...
                    observedValue: typeof body.observed_value === "number" ||
            ...
            if (method === "POST" &&
                ...
                parts[3] === "override") {
                const body = asRecord(await readJson(request));
                const rawStatus = String(body.new_status ?? body.newStatus ?? "UNKNOWN");
                const allowed = [
                    "VERIFIED",
                    "CORROBORATED",
                    "REPORTED",
                ...
                const newStatus = allowed.includes(rawStatus)
                    ? rawStatus
                    : "UNKNOWN";
            ...
            if (method === "GET" &&
                ...
                if (!evidence)
                    return problem(response, 404, "Evidence not found.");
            ...
            if (method === "GET" &&
                ...
                if (!memo)
                    return problem(response, 404, "Memo not found.");
            ...
            if (method === "GET" &&
                ...
                parts[1] === "ledger" &&
                parts[2] === "export") {
                send(response, 200, app.exportLedger(url.searchParams.get("stream") ?? ""));
                return;
            }
            if (method === "GET" &&
                parts[0] === "v1" &&
