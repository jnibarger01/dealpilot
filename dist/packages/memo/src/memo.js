...
export function composeMemo(input) {
    ...
    sections.push({
        ...
        text: input.claims
            .map((claim) => `${claim.type} is ${claim.status} ${citedList([claim.id])}.`)
            .join(" "),
...
export function citationCheck(memo) {
    const errors = [];
    for (const section of memo.sections) {
        ...
        for (const sentence of sentences) {
            ...
            const hasClaimRef = section.claimIds.some((claimId) => sentence.includes(`[${claimId}]`));
            if (!hasClaimRef) {
                errors.push(`Uncited factual sentence in ${section.heading}: ${sentence}`);
            }
        }
    }
