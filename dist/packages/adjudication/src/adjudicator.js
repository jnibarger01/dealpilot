...
const supportedTiers = ["A", "B", "C"];
function valuesAgree(claim, evidence) {
    if (typeof claim.proposition.value === "number" &&
    ...
    return (String(claim.proposition.value).trim().toLowerCase() ===
        String(evidence.observedValue).trim().toLowerCase());
}
function independentSources(evidence) {
    return [...new Set(evidence.map((item) => item.lineage ?? item.sourceId))];
}
export function adjudicateClaim(claim, evidence, rubricVersion, now = claim.updatedAt) {
    const relevant = evidence.filter((item) => item.claimId === claim.id);
    if (relevant.length === 0) {
        const row = fallbackFor(claim.type);
        const unresolved = row.ifAllFail === "REPORTED" || row.ifAllFail === "REPORTED_FLAGGED"
            ? "REPORTED"
            : "UNKNOWN";
        return {
            ...claim,
            status: unresolved,
            statusReason: unresolved === "UNKNOWN"
    ...
    const supports = relevant.filter((item) => item.supports === "support" && valuesAgree(claim, item));
    ...
    const tierA = supports.find((item) => item.sourceTier === "A" && item.method !== "user_supplied");
    if (tierA) {
        return {
            ...claim,
            status: "VERIFIED",
    ...
    const corroborating = supports.filter((item) => supportedTiers.includes(item.sourceTier) && item.sourceTier !== "D");
    ...
    const sellerOnly = relevant.every((item) => item.sourceTier === "D" || item.method === "user_supplied");
    if (sellerOnly) {
        return {
            ...claim,
            status: "REPORTED",
            statusReason: "Only seller/user-supplied tier D evidence is present; cannot upgrade beyond REPORTED.",
