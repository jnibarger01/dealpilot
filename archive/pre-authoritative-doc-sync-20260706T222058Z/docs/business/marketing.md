...
- **DevRel (mo 18+ with API):** docs-first culture (see `../engineering/api.md`), quickstarts for "verify a listing in 10 lines," MCP server so agent builders can call verification as a tool, sample apps, office hours. DevRel goal: 100 external apps calling the API within 12 mo of GA.
