...
const money = (value: string): number => Number(value.replace(/[$,\s]/g, ""));
const textValue = (value: string): string => value.trim();

const PATTERNS: readonly ExtractPattern[] = [
  {
...
export function extractClaims(
  ...
  for (const pattern of PATTERNS) {
    ...
    claims.push({
      ...
      status: "REPORTED",
      statusReason:
        "Extracted from seller-controlled artifact; born REPORTED by doctrine.",
      evidenceRefs: [],
      createdAt: now,
      updatedAt: now,
    });
